...
We don't use a real E2B sandbox in unit tests. The fake sandbox below returns
controlled ExecutionResult values per call so we can assert the flakiness
...
def test_failure_returns_immediately(offline_creds, sample_gap, sample_draft):
    sandbox = _FakeSandbox([_make_result(success=False)])
    result = ExecutionRunner(offline_creds).run(sample_draft, sample_gap, sandbox)
    assert sandbox.calls == 1
    assert result.execution_success is False


def test_three_passes_returns_non_flaky(offline_creds, sample_gap, sample_draft):
...
def test_inconsistent_runs_marked_flaky(offline_creds, sample_gap, sample_draft):
